...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if insect.can_contain(self.ant) == True:
                    insect.contained_ant, self.ant = self.ant, insect
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if hasattr(insect, 'queen') == True and insect.queen == True:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        k = 0
        while self.min_range > k or self.max_range < k:
            check_place = check_place.entrance
            k += 1
        while check_place.bees == [] and check_place.entrance != hive:
            check_place = check_place.entrance
            k += 1
        if k > self.max_range:
            return
...
class LongThrower(ThrowerAnt):
    ...
    min_range = 5
    max_range = float('inf')
    implemented = True   # Czhange to True to view in the GUI
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            new_bees_list = list(self.place.bees)
            for bees in new_bees_list:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        new_bees_list_2 = list(self.place.bees)
        for bee in new_bees_list_2:
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        if can_contain(ant) == True:
            self.contained.ant = ant
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        if self.contained_ant != None:
            self.contained_ant.action(colony)
        new_bees_list_3 = list(self.place.bees)
        for bee in new_bees_list_3:
            bee.reduce_armor(self.damage)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if self.bees != None or self.ant != None:
            for bee in self.bees:
                if bee.is_watersafe == False:
                    bee.reduce_armor(bee.armor)
            if self.ant != None and self.ant.is_watersafe == False:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        if self.created_queen == True:
            QueenAnt.created_queen = False
            self.queen = True
        else:
            self.queen = False
    ...
    def action(self, colony):
        ...
        ants_behind_new = []
        while queen_place.exit != None:
            ants_behind_new += [queen_place.exit.ant]
            queen_place = queen_place.exit
        for ant in ants_behind_new:
            if ant != None:
                if ant.is_container == True and (ant not in self.ants_behind):
                    if ant.contained_ant == None:
                        ant.damage = ant.damage * 2
                        self.ants_behind.append(ant)
                    elif ant.contained_ant not in self.ants_behind:
                        ant.contained_ant.damage, ant.damage = ant.contained_ant.damage * 2, ant.damage * 2
                        self.ants_behind.append(ant)
                        self.ants_behind.append(ant.contained_ant)
                elif ant.is_container == True and (ant in self.ants_behind):
                    if ant.contained_ant != None and (ant.contained_ant not in self.ants_behind):
                        ant.contained_ant.damage = ant.contained_ant.damage * 2
                        self.ants_behind.append(ant.contained_ant)
                elif ant not in self.ants_behind:
                    ant.damage = ant.damage * 2
                    self.ants_behind.append(ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.queen == True:
            bees_win()
...
def make_scare(action, bee):
    ...
    direction_turn = 0
    change = True
    while direction_turn < 2:
        if change == True:
            bee.place.exit = bee.place.entrance
            change = False
        direction_turn += 1
    bee.place.exit = bee.place.entrance
