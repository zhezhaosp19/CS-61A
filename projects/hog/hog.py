...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    p = player_score % 100
    o = opponent_score % 100
    return abs((p//10) - (p%10)) == abs((o//10) - (o%10))
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def current_score(score0,score1):
        if who:
            score = score1
        else:
            score = score0
        ...
        if dif > previous_high:
            print(dif, "point(s)! That's the biggest gain yet for Player", who)
            return announce_highest (who, dif, score)
        else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    avrg = 0
    while num <= 10:
        a = make_averaged(roll_dice, num_samples)(num, dice)
        if avrg < a:
            avrg = a
            k = num
        num = num + 1
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    n = (2 * (opponent_score//10)) - (opponent_score % 10)
    if n>1:
        a = n
    else:
        a = 1
