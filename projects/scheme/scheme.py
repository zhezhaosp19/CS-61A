...
def scheme_eval(expr, env, tail = False): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        new_first = scheme_eval(first, env)
        if isinstance(new_first, MacroProcedure):
            return scheme_eval(new_first.apply_macro(rest, env), env)
        new_rest = rest.map(lambda new_expr: scheme_eval(new_expr, env))
        return scheme_apply(new_first, new_rest, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil or vals is not nil:
            if (formals is nil and vals is not nil) or (formals is not nil and vals is nil):
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lamb_instance = do_lambda_form(Pair(expressions.first.second, expressions.second), env)
        env.define(target.first, lamb_instance)
        return target.first
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    formals = expressions.first
    #check_formals(formals)
    if isinstance(formals, Pair) and scheme_symbolp(formals.first):
        macro_instance = MacroProcedure(formals.second, expressions.second, env)
        env.define(formals.first, macro_instance)
