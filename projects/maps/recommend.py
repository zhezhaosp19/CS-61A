...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    all_restaurant = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
    return group_by_first(all_restaurant)
    # END Question 4
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(r) for r in cluster]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared = Sxy / Sxx, mean(ys) - Sxy / Sxx * mean(xs), pow(Sxy, 2) / (Sxx * Syy)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    predictor_r_squared_lst= [find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]
    return max(predictor_r_squared_lst, key = lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            dic_rate[name] = user_rating(user, name)
        else:
            dic_rate[name] = predictor(r)
    return dic_rate
    # END Question 9
