...
def balanced(m):
    ...
    if is_weight(m):
        ...
        if balanced(end(left(m))) and balanced(end(right(m))) and l == r:
            return True
        else:
            return False
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    num_wrong_password = []
    def withdraw2(amount, password_input):
        nonlocal balance
        if len(num_wrong_password) >= 3:
            return "Your account is locked. Attempts: " + str(num_wrong_password)
        elif password_input != password:
            num_wrong_password.append(password_input)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(value) == str:
        ...
        def withdraw3(amount, password_input):
            if password_input == old_password or password_input == new_password:
...
def generate_paths(t, x):
    ...
    if label(t) == x:
      yield [x]
    for b in branches(t):
        iterator_rests_of_paths = generate_paths(b, x)
        for rest_of_path in iterator_rests_of_paths:
